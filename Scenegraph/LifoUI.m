...
@interface LifoUI ()
// Make guiHandle writable privately if protocol/header is readonly.
@property (nonatomic, strong, readwrite) NSView *guiHandle;
@end
...
// If the protocol requires a factory/initializer, keep it here.
// Example: create the main NSView the first time it’s asked for.
- (NSView *)guiHandle {
    if (!_guiHandle) {
        NSView *root = [[NSView alloc] initWithFrame:NSMakeRect(0, 0, 792, 310)];
        
        // lifoPanel
        self.lifoPanel = [[NSView alloc] initWithFrame:NSMakeRect(8, 8, 768, 288)];
        [root addSubview:self.lifoPanel];
        
        NSComboBox *combo = [[NSComboBox alloc] initWithFrame:NSMakeRect(16, 16, 200, 26)];
        combo.usesDataSource = NO;
        [root addSubview:combo];
        self.cbitem = combo;
        
        [self populateFormatComboBox]; // safe here—combo exists now
                                       // Build controls and menus
        [self setupUI];          // assumes it uses self.lifoPanel / subviews of root
        [self setupContextMenu]; // ditto
...
- (void)populateFormatComboBox {
    ...
    [self.cbitem addItemWithObjectValue:@(TxtrFormatsUnknown)];
    [self.cbitem addItemWithObjectValue:@(TxtrFormatsDXT1Format)];
    [self.cbitem addItemWithObjectValue:@(TxtrFormatsDXT3Format)];
    [self.cbitem addItemWithObjectValue:@(TxtrFormatsDXT5Format)];
    [self.cbitem addItemWithObjectValue:@(TxtrFormatsRaw8Bit)];
    [self.cbitem addItemWithObjectValue:@(TxtrFormatsRaw24Bit)];
    [self.cbitem addItemWithObjectValue:@(TxtrFormatsRaw32Bit)];
}
...
- (IBAction)btexClick:(id)sender {
    ...
    // Ensure our view exists (and is attached if already shown)
    (void)self.guiHandle;
    
    NSWindow *win = self.guiHandle.window ?: NSApp.keyWindow;
    if (win) {
        [self.sfd beginSheetModalForWindow:win
        ...
        // Fallback if we're not in a window yet: app-modal panel
        [self.sfd beginWithCompletionHandler:^(NSInteger result) {
            if (result == NSModalResponseOK) {
                NSURL *url = self.sfd.URL;
                NSData *imageData = [self.pb.image TIFFRepresentation];
                NSBitmapImageRep *imageRep = [NSBitmapImageRep imageRepWithData:imageData];
...
- (IBAction)btimClick:(id)sender {
    // Ensure our editor view exists (and grab its window if embedded)
    (void)self.guiHandle;
    NSWindow *win = self.guiHandle.window ?: NSApp.keyWindow;
    
    void (^handleResult)(NSInteger) = ^(NSInteger result) {
        if (result == NSModalResponseOK) {
            @try {
    ...
    if (win) {
        [self.ofd beginSheetModalForWindow:win completionHandler:handleResult];
    } else {
        [self.ofd beginWithCompletionHandler:handleResult];
    }
}
...
- (IBAction)importAlpha:(id)sender {
    // Ensure the editor view exists and fetch its window
    (void)self.guiHandle;
    NSWindow *win = self.guiHandle.window ?: NSApp.keyWindow;
...
- (IBAction)exportAlpha:(id)sender {
    ...
    // Ensure our editor view exists and get its window
    (void)self.guiHandle;
    NSWindow *win = self.guiHandle.window ?: NSApp.keyWindow;
    ...
    void (^handleResult)(NSInteger) = ^(NSInteger result) {
        if (result == NSModalResponseOK) {
            @try {
                ...
                NSData *tiff = [alphaImage TIFFRepresentation];
                if (!tiff) return;
    ...
    if (win) {
        [self.sfd beginSheetModalForWindow:win completionHandler:handleResult];
    } else {
        [self.sfd beginWithCompletionHandler:handleResult];
    }
}
...
- (IBAction)importDDS:(id)sender {
    ...
    ddsPanel.allowsMultipleSelection = NO;
    ddsPanel.canChooseDirectories = NO;
    ...
    if (win) {
        [ddsPanel beginSheetModalForWindow:win completionHandler:handleResult];
    } else {
        [ddsPanel beginWithCompletionHandler:handleResult];
    }
}
...
- (void)encodeWithCoder:(NSCoder *)coder {
    // No state needs to be archived
}

- (NSView *)createView {
    // build lazily if needed and return the plugin's root view
    return self.guiHandle;
}

- (void)refresh {
    // Ensure the view (and cbitem) exists if you're building UI lazily
    (void)self.guiHandle;
    
    // Update the GUI with current wrapper data
    if (self.wrapper != nil) {
        // Clear and repopulate the item combo box
        [self.cbitem removeAllItems];
        
        for (LevelInfo *levelInfo in self.wrapper.blocks) {
            [self.cbitem addItemWithObjectValue:levelInfo];
        }
...
- (void)synchronize {
    ...
    // Read selection
    NSInteger idx = self.cbitem.indexOfSelectedItem;
    if (idx < 0) return;
    
    // Get blocks via KVC (avoids needing a visible -blocks declaration)
    NSArray *blocks = nil;
    @try { blocks = [self.wrapper valueForKey:@"blocks"]; } @catch (__unused NSException *e) {}
    if (blocks.count == 0 || idx >= (NSInteger)blocks.count) return;
    
    id levelInfo = blocks[idx];
    
    // Try common keys; each @try is independent and harmless if the key doesn't exist
    @try { [self.wrapper setValue:levelInfo forKey:@"selectedBlock"]; } @catch (__unused NSException *e) {}
    @try { [self.wrapper setValue:levelInfo forKey:@"currentBlock"];  } @catch (__unused NSException *e) {}
    @try { [self.wrapper setValue:@(idx)    forKey:@"selectedIndex"]; } @catch (__unused NSException *e) {}
}
// MARK: - Update GUI Method (matches C# signature)

// NOTE: IPackedFileUI requires updateGUI:, exact capitalization.
- (void)updateGUI:(id<IFileWrapper>)wrapper {
    // ensure the view exists if you build lazily
    (void)self.guiHandle;
    
    // store the wrapper on this UI (no more self.form)
    self.wrapper = (Lifo *)wrapper;   // or cast to LifoWrapper if that's the name
